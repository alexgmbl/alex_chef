...
2. Select the **AlexChef** scheme and choose an iOS Simulator (e.g., iPhone 15).
...
GitHub Actions runs `xcodebuild` against the project on pushes and pull requests targeting `main` or `develop`. The workflow file is located at `.github/workflows/ios-ci.yml` and uses `macos-latest` runners with Xcode 15.2.
